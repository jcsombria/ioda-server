...
class InteractiveTask(TemplateView):
    ''' Submit an interactive job.
    '''
    template_name = 'task/interactive.html'
...
#                 content = job_input.read()
#                 encoded = base64.b64encode(content).decode('ascii')
#                 result = send_task("mytasks.echo", [str(encoded)])
#                 response = result.get()
#                 web = '<img src="data:image/png;base64,' + response + '" alt="Red dot" />'
#                 return HttpResponse(web)

        content = request.body
        print(content)
        result = send_task("mytasks.echo", [content])
        response = result.get()
