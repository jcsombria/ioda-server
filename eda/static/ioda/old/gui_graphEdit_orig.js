...
//				'  onblur="sMainCommandCenter.sendCommand(\''+ mID+'\',\'#{command}\')" '+
	...
	const mGRAPH_OPTIONS_HTML =
		...
		'<div class="d-flex flex-column" style="flex-grow:0; flex-basis:0;">'+
			'<div id="'+ mID+'-graph_options_div" class="h-100" style="flex-grow:0; flex-basis:0;">'+
			'</div>'+
		'</div>'+
		// --- Main
		'<div class="d-flex flex-column overflow-scroll mb-1" style="flex-grow:1; flex-basis:0;">'+
			'<div id="'+ mID+'-graph_utils_div" class="h-100 overflow-scroll" style="flex-grow:1">'+
			'</div>'+
		'</div>'+
...
/*		
		...
		'<div class="d-flex flex-column overflow-scroll" style="flex-grow:1; flex-basis:0;">'+
			...
			'</div>'+
			'<div id="'+ mID+'-graph_utils_div" class="h-100 overflow-scroll" style="flex-grow:0">'+
			'</div>'+
		'</div>'+
*/
		'<div class="sTabRight_header border border-dark bg-light p-2 d-flex flex-row" style="flex-grow:0; flex-basis:0">'+
	...
	var mGraphRunInfoDiv = $('#'+ mID+'-graph_utils_div')
	
	self.displayRunInfo = function(nodeInfo) {	
		mGraphRunInfoDiv.empty();
		if (nodeInfo!=null) {
			var node = mGraph.findNode(nodeInfo.node);
			mGraphRunInfoDiv.append(mMIDDLE_HEADER_LINE
				.replace( /#\{name\}/g, sMainRes.getString('Node run') ));
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_name') )
				.replace( /#\{name\}/g, sMainRes.getString('Name') )
				.replace( /#\{value\}/g, node.getName() ));			
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_code') )
				.replace( /#\{name\}/g, sMainRes.getString('Code') )
				.replace( /#\{value\}/g, nodeInfo.code ));			
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_start') )
				.replace( /#\{name\}/g, sMainRes.getString('Started') )
				.replace( /#\{value\}/g, nodeInfo.start ));			
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_end') )
				.replace( /#\{name\}/g, sMainRes.getString('Ended') )
				.replace( /#\{value\}/g, nodeInfo.end ));			
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_lapsed') )
				.replace( /#\{name\}/g, sMainRes.getString('Lapsed') )
				.replace( /#\{value\}/g, nodeInfo.lapsed ));			
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_url') )
				.replace( /#\{name\}/g, sMainRes.getString('Output') )
				.replace( /#\{value\}/g, nodeInfo.output ));				
		}
		else {
			mGraphRunInfoDiv.append(mMIDDLE_HEADER_LINE
				.replace( /#\{name\}/g, sMainRes.getString('Graph run') ));
			mGraphRunInfoDiv.append(mINFO_LINE.replace( /#\{label\}/g, (mID+'-run_code') )
				.replace( /#\{name\}/g, sMainRes.getString('Code') )
	...
	self.showGraphOptions = function() {
		...
		if (!mIsRunReplay) {
			mGraphOptionsDiv.append(mCHECK_FIELD.replace( /#\{label\}/g, (label+'simulate_conditionals') )
				...
				.replace( /#\{extras\}/g, '')
				.replace( /#\{checked\}/g, mRunControl.getSimulateConditionals() ? 'checked' : ''));
			mGraphOptionsDiv.append(mCHECK_FIELD.replace( /#\{label\}/g, (label+'true_conditionals') )
				...
				.replace( /#\{extras\}/g, '')
				.replace( /#\{checked\}/g, mRunControl.getSimulateTrueConditionals() ? 'checked' : ''));
			mGraphOptionsDiv.append('<div class="h-divider"></div>');
		}
		else self.displayRunInfo(null);
	...
	self.graphOptionsCommand = function(command) {
		...
		switch (command) {
			...
			case 'display_style' :
				...
				break;
			case 'start' :
				mRunControl.start(self);
				break;
			case 'step' :
				mRunControl.step(self);
				break;
			case 'stop' :
				mRunControl.stop(self,true);
				break;
