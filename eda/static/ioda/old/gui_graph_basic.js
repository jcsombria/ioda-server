 ...
 */
IODA_GUI.graphBasic = function(mID, mTitle, mParent, mEditable) {
	...
	self.setRunControl = function(control) { mRunControl = control; }
	...
	self.saveObject = function() {
		...
		if (mRunControl) object.run_control = mRunControl.toObject();
		return object;
	}
...
//				'  onblur="sMainCommandCenter.sendCommand(\''+ mID+'\',\'#{command}\')" '+
	...
	const mGRAPH_OPTIONS_HTML =
		...
		'<footer class="border border-dark bg-light d-flex flex-row">'+
			'<div id="'+ mID+'-graph_options_footer_left_div" class="container-fluid p-0 justify-content-start" style="flex-grow:1">'+
				...
				'</button>'+
			'</div>'+
			'<div id="'+ mID+'-graph_options_footer_right_div" class="container-fluid p-2 justify-content-end" style="flex-grow:0; flex-basis:0">'+
			'</div>'+
		'</footer>'; 
	...
	const graphOptionLabel = mID+"-graph_option-";
	...
	// fill mGraphDiv
	(function() { 
		var mGraphOptionsDiv = self.getGraphOptionsDiv();
		var mGraphCommentDiv = $('#'+ mID+'-graph_comment_div');

//		mGraphOptionsDiv.empty();
//		mGraphCommentDiv.empty();
	
		mGraphOptionsDiv.append(IODA_GUI.GRAPH.Constants.CHECK_FIELD
			.replace( /#\{label\}/g, (graphOptionLabel+'node_labels') )
		...
		mGraphOptionsDiv.append(IODA_GUI.GRAPH.Constants.CHECK_FIELD
			.replace( /#\{label\}/g, (graphOptionLabel+'output_labels') )
		...
		mGraphOptionsDiv.append(IODA_GUI.GRAPH.Constants.CHECK_FIELD
			.replace( /#\{label\}/g, (graphOptionLabel+'input_labels') )
		...
		mGraphOptionsDiv.append(IODA_GUI.GRAPH.Constants.CHECK_FIELD
			.replace( /#\{label\}/g, (graphOptionLabel+'condition_labels') )
	...
	self.fillGraphOptions = function() {
		$('#'+graphOptionLabel+'node_labels').prop('checked', mGraph.getShowNodeLabels());
		$('#'+graphOptionLabel+'output_labels').prop('checked', mGraph.getShowOutputLabels());
		$('#'+graphOptionLabel+'input_labels').prop('checked', mGraph.getShowInputLabels());
		$('#'+graphOptionLabel+'condition_labels').prop('checked', mGraph.getShowConditionLabels());
	...
	self.graphCommand = function(command) {
		...
		//var value  = document.getElementById(graphOptionLabel+command).checked;
		switch (command) {
			case 'node_labels' : 
				mGraph.setShowNodeLabels($('#'+graphOptionLabel+command).prop('checked')); 
				break;
			case 'input_labels'     : 
				mGraph.setShowInputLabels($('#'+graphOptionLabel+command).prop('checked')); 
				break;
			case 'output_labels'    : 
				mGraph.setShowOutputLabels($('#'+graphOptionLabel+command).prop('checked')); 
				break;
			case 'condition_labels' : 
				mGraph.setShowConditionLabels($('#'+graphOptionLabel+command).prop('checked')); 
				break;
			...
			case 'start' : mRunControl.start();    break;
			case 'step'  : mRunControl.step();     break;
			case 'stop'  : mRunControl.stop(true); break;
	...
	const mPROPERTIES_HTML =
		...
		'<div class="d-flex flex-column" style="flex-grow:0; flex-basis:0;">'+
			'<div id="'+ mID+'-node_name_div" class="h-100" style="flex-grow:0; flex-basis:0;">'+
			'</div>'+
		'</div>'+
		// --- Main
		'<div class="d-flex flex-column overflow-scroll mb-1" style="flex-grow:1; flex-basis:0;">'+
			'<div id="'+ mID+'-node_properties_div" class="h-100 overflow-scroll" style="flex-grow:1">'+
			'</div>'+
		'</div>'+
		...
		// --- Footer
		'<footer class="border border-dark bg-light d-flex flex-row">'+
			'<div id="'+ mID+'-node_options_footer_left_div" class="container-fluid p-0 justify-content-start" style="flex-grow:1">'+
				'<button class="btn btn-link p-0 text-decoration-none" type="button"'+
					'onclick="sMainCommandCenter.sendCommand(\''+ mID+'\',\'info\')">'+
					'<i class="bi bi-info-square"></i>'+
				'</button>'+
			'</div>'+
			'<div id="'+ mID+'-node_options_footer_right_div" class="container-fluid p-2 justify-content-end" style="flex-grow:0; flex-basis:0">'+
			'</div>'+
		'</footer>';
...
//		style: "flex-grow:0; flex-basis:0;", 
	...
	const nodeOptionLabel = mID+"-node_option-";
		
	// Used by subclasses
	self.getNodeFooterLeftDiv  = function() { return $('#'+ mID+'-node_options_footer_left_div'); }
	self.getNodeFooterRightDiv = function() { return $('#'+ mID+'-node_options_footer_right_div'); }

	// fill mGraphDiv
	(function() { 
		var mClassAndNameDiv   = $('#'+ mID+'-node_name_div');
		var mPropertiesListDiv = $('#'+ mID+'-node_properties_div');
		...
		properties.forEach(function(property) {
			...
			if (node.getPropertyConnectionLinks(property)<=0 && node.isPropertyType(property,'OPTION')) {
				...
				var propertyHtml = mSELECT_LINE.replace(/#\{label\}/g,(nodeOptionLabel+property.name))
				...
				var optionsSelect = $('#'+nodeOptionLabel+property.name);
				optionsSelect.empty();
				...
				if(node.isPropertyAttribute(property,'manual')) {
					var propertyHtml = mPROPERTY_LINE
						.replace(/#\{label\}/g,(nodeOptionLabel+property.name))
						.replace( /#\{name\}/g, property.name)
						.replace( /#\{command\}/g, 'property_change:'+ property.name)
				...
				if(node.isPropertyAttribute(property,'input') && (connectionLinks.length>0 || !node.isPropertyAttribute(property,'manual')) ) {
					var propertyHtml = mLINKS_LINE
						.replace(/#\{label\}/g,(nodeOptionLabel+property.name+'-links'))
